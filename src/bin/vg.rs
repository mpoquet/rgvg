...
const TOOLS: &'static str = r"vscode: code -g {file}:{line}
pico: pico +{line} {file}
nano: nano +{line} {file}
vim: vim +{line} {file}
...
fn get_tool(tool_list: Vec<&str>, tool: String) -> String {
    ...
    let tool_def = tool_list
        .iter()
        .find(|c| r.is_match(c))
        .expect("No tool with requested name!");
    return r.replace(&tool_def, "").to_string();
}

fn open(tool_list: String, id: usize, tool: String) {
    let (mut r, s) =
        rgvg::common::open_last().expect("No last file for user! Use 'cg' to create a last file.");
    let request = s
        .get(id)
        .expect("Delivered id was not valid! Valid IDs range between 0..MAX_ID.");

    r.push(request.filename.clone());

    let t = get_tool(tool_list.split('\n').collect(), tool);
    let mut t: Vec<String> = t
        .split(" ")
        .map(|s| {
            ...
            return r2
                .replace(
                    &r1.replace(s, r.to_string_lossy()).to_string(),
    ...
    let c = (
        t.get(0).expect("Could not find executable!").to_string(),
        t[1..].to_vec(),
    );
...
fn add_tool(nt: String) {
    let h = home::home_dir()
        ...
        .join(rgvg::common::OPEN_FORMAT_PATH);
    if !h.exists() {
        std::fs::write(h, TOOLS.to_owned() + &nt + "\n")
            .expect("Could not create tool registry in your home directory!")
    } else if h.is_file() {
        let mut e = std::fs::OpenOptions::new()
            .append(true)
            .open(h)
            .expect("Could not edit tool registry!");
